...
for bootstrap_num in np.arange(bootstrap_num_start, (bootstrap_num_end+1)):
	...
	# split training and testing loc
	np.random.seed(seed = bootstrap_num)
	train_loc = np.random.choice(np.arange(0, len(currentdata_x[:, 0])), size = round((1-nn_split_ratio)*len(currentdata_x[:, 0])), replace = True)
