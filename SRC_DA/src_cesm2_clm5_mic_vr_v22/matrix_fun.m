...
% tau4l3 = tau4l2;
% % tau for DOC (yr) 1.1*10-2
% tau4s1 = 10^(kp(8)*((0) - (-3)) + (-3)); % kp(8)*((0.02) - (0)) + (0); % 
% % tau for MIC (yr) 0.57 for death
% tau4s2_death = 10^(kp(9)*((0) - (-2)) + (-2)); % kp(9)*(0.3 - (0.0)) + (0.0); % 
% % tau for MIC (yr) 22 for enz production
% tau4s2_enz = 10^(kp(10)*(2 - (0)) + (0)); % kp(10)*(30 - (15)) + (15); % 
% % tau for ENZ (yr) 0.11
% tau4s3 = 10^(kp(11)*((0) - (-2)) + (-2)); % kp(11)*(0.3 - 0) + 0; % 
% % tau for SOC (yr)  default: 4.6*10^-5 -- 1.1*10^-4
% tau4s4 = 10^(kp(12)*((-3) - (-6)) + (-6)); % kp(12)*(3*10^(-4) - 0) + 0; % 
% % ------------michaelis-menten
% % (michaelis-menten) concentration DOC (yr) for half max assimlation reaction from DOC to MIC, unit: gc/m3, 4*10^2
% mm_const_assim = 10^(kp(13)*(3 - 0) + 0); % kp(13)*(1000 - 100) + 100; %  
% % (michaelis-menten) concentration DOC (yr) for half max decomposition reaction from SOC to MIC, unit: gc/m3, 5*10^(4) -- 6*10^5
% mm_const_decom = 10^(kp(14)*(9 - 5) + 5); % kp(14)*(10^6 - 10^5) + 10^5; %
% % ------------transfer fraction intra litter
% % fraction from cwd to l2 0.75
% fcwdl2 = kp(15)*(1 - 0.5) + 0.5;
% % ------------transfer fraction litter -> soil
% pl1s1 = kp(16)*(0.5 - 0) + 0;
% pl2s1 = kp(17)*(0.8 - 0.3) + 0.3;
% pl3s4 = kp(18)*(1 - 0.5) + 0.5;
% 
% % fraction from l1 to s2 0.45
% l1_cue = kp(19)*(0.8 - 0.2) + 0.2;
% l2_cue = kp(20)*(0.8 - 0.2) + 0.2;
% l3_cue = kp(21)*(0.8 - 0.2) + 0.2;
% 
% fl1s1 = l1_cue*pl1s1;
% fl1s2 = l1_cue*(1 - pl1s1); % kp(19)*(0.7 - 0.0) + 0.0; % 
% 
% fl2s1 = l2_cue*pl2s1;
% fl2s2 = l2_cue*(1 - pl2s1); % kp(20)*(0.4 - 0.0) + 0.0; % 
% 
% fl3s2 = l3_cue*(1 - pl3s4); % kp(21)*(0.1 - 0.0) + 0.0; % 
% fl3s4 = l3_cue*pl3s4;
% 
% % ------------transfer fraction intra soil
% % cue
% mic_cue = kp(22)*(0.7 - 0.01) + 0.01;
% % fraction from s1 to s2
% fs1s2 = mic_cue;
% % fraction death to soc
% pdeath2soc = kp(23)*(0.05 - 0) + 0;
% % fraction from enz to doc
% fs3s1 = 1;
% % fraction from soc to doc
% fs4s1 = 1;
