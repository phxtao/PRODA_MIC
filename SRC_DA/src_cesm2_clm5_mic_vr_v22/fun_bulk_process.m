function [carbon_input, cpools, cpools_layer, soc_layer, bulk_A, bulk_K, bulk_V, bulk_Xi, bulk_I, bulk_E] = ...
...
if isempty(find(cpools < 0, 1)) == 0
    ...
    cpools = nan(size(cpools));
    soc_stock = nan(size(soc_stock));
    soc_layer = nan(size(soc_layer));
    ...
    %% bulk process
    decom_cpool_apparent = [1/(tau4cwd)*monthly_mean_xi_wt, ... 
        1/(tau4l1)*monthly_mean_xi_wt, ... 
        1/(tau4l2)*monthly_mean_xi_wt, ... 
        1/(tau4l3)*monthly_mean_xi_wt, ...
        (1/(tau4s1) .* monthly_mean_xi_wt).*mic_ss./(mm_const_assim .*monthly_mean_xi_wt + doc_ss), ...
        (1/tau4s2_death + 1/tau4s2_enz)*monthly_mean_xi_wt, ...
        1/(tau4s3)*ones(n_soil_layer, 1), ... 
        (1/(tau4s4) .* monthly_mean_xi_wt).*enz_ss./(mm_const_decom .*monthly_mean_xi_wt + soc_ss) 
        ]; % unit yr-1
    ...
    % K    
    %     partial_soc_0 = 1/(tau4s4).*enz_ss.*mm_const_decom.* ...
    %         (2.*mm_const_decom.*soc_ss.*monthly_mean_xi_wt.^2 + 2.*soc_ss.^2.*monthly_mean_xi_wt) ./ ...
    %         (mm_const_decom.^2*monthly_mean_xi_wt.^2 + 2.*mm_const_decom.*soc_ss.*monthly_mean_xi_wt + soc_ss.^2).^2;
    
    partial_soc = 1/(tau4s4).*enz_ss.*mm_const_decom.* ...
        2.*soc_ss.*monthly_mean_xi_wt ./ ...
        (mm_const_decom.*monthly_mean_xi_wt + soc_ss).^3;
    
    decom_cpool_baseline = [1/(tau4cwd)*ones(n_soil_layer, 1), ... % transfer of cwd to two litter pools
        1/(tau4l1)*ones(n_soil_layer, 1)*pl1s1, ... % transfer of l1 to s1
        1/(tau4l2)*ones(n_soil_layer, 1)*pl2s1, ... % transfer of l2 to s1
        1/(tau4l3)*ones(n_soil_layer, 1)*pl3s4, ... % transfer of l3 to s4
        (1/tau4s2_death + 1/tau4s2_enz)*ones(n_soil_layer, 1), ... % death of mic to doc or soc
        1/(tau4s3)*ones(n_soil_layer, 1), ... % decay of enz
        partial_soc % soc decomposition
        ]; % unit yr-1
    
    cpools_total = cpools(:, [1:4, 6:8]).*repmat(dz(1:n_soil_layer), [1, 7]); % soc stock unit gc/m2
    
    bulk_K = sum(sum(decom_cpool_baseline.*(cpools_total/(sum(sum(cpools_total, 'omitnan'), 'omitnan'))), 'omitnan'), 'omitnan'); % unit yr-1
    
    % bulk_K = sum(partial_soc.*(soc_layer(:, 4).*dz(1:n_soil_layer))/sum((soc_layer(:, 4).*dz(1:n_soil_layer))));
    
    % E
    donor_pool_layer = cpools;
    
    transfer_cpool = ...
        [repmat(pl1s1, [n_soil_layer, 1]), ...
        repmat(pl2s1, [n_soil_layer, 1]), ...
        repmat(pl3s4, [n_soil_layer, 1]), ...
        repmat(pdeath2soc, [n_soil_layer, 1])        
        ];
    
    donor_pool_size = [...
        donor_pool_layer(:, 2), donor_pool_layer(:, 3), donor_pool_layer(:, 4), ...
        donor_pool_layer(:, 6)...
        ];
    
    donor_decomp = [...
        decom_cpool_apparent(:, 2), decom_cpool_apparent(:, 3), decom_cpool_apparent(:, 4), ...
        decom_cpool_apparent(:, 6)...
        ];
    
    total_doner_flow = nan(n_soil_layer, length(transfer_cpool(1, :)));
    
    for idoner = 1:length(transfer_cpool(1, :))
        total_doner_flow(:, idoner) = sum(donor_pool_size(:, idoner).*donor_decomp(:, idoner).*dz(1:n_soil_layer));
    end
    
    bulk_E = sum(sum(transfer_cpool.*total_doner_flow))/sum(sum(total_doner_flow(:, [1, 2, 3, 4])));
    
    % bulk_E = 1/tau4s2_death*pdeath2soc;
    
    % xi
    bulk_Xi_layer = mean(xit, 2).*mean(xiw, 2).*depth_scalar;
    
    bulk_Xi = sum(bulk_Xi_layer.*(soc_layer(:, 5).*dz(1:n_soil_layer))/sum((soc_layer(:, 5).*dz(1:n_soil_layer))));
    ...
    cue_cpool = ...
        [repmat(fl1s2, [n_soil_layer, 1]), ...
        repmat(fl2s2, [n_soil_layer, 1]), ...
        repmat(fl3s2, [n_soil_layer, 1]), ...
        repmat(fs1s2, [n_soil_layer, 1]), ...
        ];
    ...
    donor_decomp = [...
        decom_cpool_apparent(:, 2)*(1-pl1s1), decom_cpool_apparent(:, 3)*(1-pl2s1), decom_cpool_apparent(:, 4)*(1-pl3s4), ...
        decom_cpool_apparent(:, 5)...
        ];
    
    total_doner_flow = nan(n_soil_layer, length(cue_cpool(1, :)));
    
    for idoner = 1:length(cue_cpool(1, :))
        total_doner_flow(:, idoner) = sum(donor_pool_size(:, idoner).*donor_decomp(:, idoner).*dz(1:n_soil_layer));
    end
    
    bulk_A = sum(sum(cue_cpool.*total_doner_flow))/sum(sum(total_doner_flow(:, [1, 2, 3, 4])));
    
    % V
    bulk_V_monthly = nan(month_num, 1);
    for imonth = 1:month_num
        bulk_V_middle = diag(tri_ma_middle(21:40, 21:40, imonth))*365; % unit from day-1 to yr-1
        bulk_V_monthly(imonth) = sum(bulk_V_middle.*(sum(cpools(:, 2:4), 2).*dz(1:n_soil_layer))/sum((sum(cpools(:, 2:4), 2).*dz(1:n_soil_layer))));
    end
